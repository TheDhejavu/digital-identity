...
async function registerOrg() {
    try{
        ...
        let orgIdentity = value;
        const gateway = new Gateway();
        let walletPath = path.join(process.cwd(), 'wallet');
        const wallet = new FileSystemWallet(walletPath);

        const userExists = await wallet.exists(orgIdentity);
        if(userExists){
        ...
        wallet.import(orgIdentity, identity);
        console.log('Successfully registered and enrolled admin organisation ' + orgIdentity + ' and imported it into the wallet');
